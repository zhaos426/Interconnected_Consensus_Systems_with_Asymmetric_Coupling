...
#    print((x0))
    ...
    for i in range(len(deltas)):
        ...
        ax[3].semilogy(dt*arange(tt), errors[:,i])
    ax[0].set_ylabel('$x_p(t)$')
    ax[3].set_ylabel('$||x(t) -\pi ||$')
...
## fig 3 ##
######################################################
def plot_lambda2_vs_delta_chis(network_name,deltas,chis,plot_chis,axis):
    """
...
def plot_dlambda2_ddelta(network_name,omegas,deltas,chis,axis):
    """
        plotting the predicted convergence rate versus delta for different choices of chis
    ...
    lambda2s_exact = get_lambda2_2layer_exact(network_name,omegas,deltas,chis) #omega*delta*chi
    dlambda2_ddelta_exact = (lambda2s_exact[:,1:,:] - lambda2s_exact[:,0:-1,:]).T / (deltas[1] - deltas[0])
    dlambda2_ddelta_thm = get_dlambda2_ddelta_thm(network_name,chis) #chi*2
...
def plot_optima(network_name,omegas,deltas,chis,axis):
...
def plot_contour(network_name,deltas,chis,axis):
    lambda2s_thm = get_lambda2_2layer_thm(network_name,deltas,chis) # delta*chi
...
## fig 4B ##
######################################################
def plot_Leval2_eachlayer(network_name,chis,axis):
    """
    Parameters
    ----------
    network_name: the name of the network
    ...
    """
    Leval2_eachlayer,Ls = get_Leval2_2layer_eachlayer(network_name,chis)
...
######################################################
def plot_ReIm_lambda23(network_name,omegas,deltas):
    """
        Parameters
        ----------
        network_name: the name of the network
