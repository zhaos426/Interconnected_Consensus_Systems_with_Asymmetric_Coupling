...
def get_network_properties(network_name,delta=0):
    """
    Parameters
    ----------
    ...
    if network_name == 'HumanAINetwork':
        ...
        count = 0
        for i in range(n):
            neighbors = np.random.permutation(n)[:d]# for node i, choose d neighbors at random
            for j in neighbors:
                EL2b[count] = [i+1,j+1]
                count +=1
        GL2b = nx.DiGraph()
